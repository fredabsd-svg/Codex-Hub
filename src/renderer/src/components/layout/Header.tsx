...
export function Header({ conversation }: { conversation: ConversationSummary | null }) {
  ...
  const modeOptions: Array<{ value: OperationMode; label: string; hint: string; disabled?: boolean; disabledReason?: string }> = [
  ...
  return (
    <header
      className="flex h-12 flex-none items-center gap-2 border-b px-3"
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        <Tooltip content={conversation?.workspacePath ?? 'Nenhum workspace vinculado. Escolher com Ctrl+O.'}>
          <Button
            size="sm"
            variant="subtle"
            iconLeft={<IconFolder />}
      ...
      <div className="flex flex-none items-center gap-1.5">
        ...
        >
          <div className="p-3 text-[12.5px] leading-relaxed">
            <h3 className="mb-1.5 text-[13px] font-semibold text-[var(--text)]">{t('header.connection')}</h3>
            {connection ? (
              <>
