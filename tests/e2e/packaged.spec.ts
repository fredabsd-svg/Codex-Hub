...
const PACKAGED = join(process.cwd(), 'release', '0.1.0', 'linux-unpacked', 'codex-hub');
...
test('o aplicativo empacotado abre, monta a interface e grava fora do ASAR', async () => {
  ...
  try {
    ...
    await expect(page.getByRole('navigation', { name: /Navegação principal/i })).toBeVisible({ timeout: 60_000 });
